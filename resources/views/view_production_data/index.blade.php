...
@section('content_header')
    <div class="col-sm">
        <h4><i class="fa-solid fa-list-ul"></i> &nbsp;&nbsp;Lista Geral - Dados de Produção</h4>
    </div>
    <div class="row p-2">
        <div class="col-sm">
            @if (session('success'))
            ...
            @if (isset($_GET['id']))
                <x-adminlte-card class="bg-warning" title=" Tem Certeza que Deseja Excluir o Arquivo?" theme="warning"
                    ...
                    <form action="view_production_data/{{ $_GET['id'] }}" method="POST">
                        @csrf
                        @method('DELETE')
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        <a class="btn btn-success btn-lg px-2 py-1"
                                                           href="{{ route('view_production_data.download', ['file' => $return_db->file]) }}"
                                                    ...
                                                    @can('is_admin')
                                                        <div class="btn-group">
                                                            <a type="submit" class="btn btn-danger btn-lg px-2 py-1"
                                                                href="view_production_data?id={{ $return_db->id }}&path={{ $return_db->path }}&file={{ $return_db->file }}"
